...
export function addTodo(label){
    return (dispatch) => {
        if(!label){
            return dispatch(sendError("Error: Can't add Todo without label"));
        }

        else{
            return dispatch( { type: Constants.ADD_TODO, label } );
        }
    }
}
...
export function updateTodoLabel(id, label){
    return (dispatch) => {
        if(!label){
            return dispatch(sendError("Error: Can't update Todo without label"));
        }

        else{
            return dispatch( {type: Constants.UPDATE_TODO_LABEL, id ,label});
        }
...
export function sendError(msg) {
    return {type: Constants.ADD_ERROR, msg}
}
